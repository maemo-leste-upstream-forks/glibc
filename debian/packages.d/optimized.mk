...
ifeq ($(NO_LOG),)
	...
	@echo 'Building optimized GNU C Library for a $(OPT_TARGET) host.'
...
$(stamp_configure_opt): $(stamp_unpack) $(stamp_patch)
	...
	cd $(objdir_opt) && CC="$(CC)" \
		$(srcdir)/configure --host=$(OPT_TARGET) \
		--build=$(DEB_BUILD_GNU_TYPE) --prefix=/usr --without-cvs \
		--disable-profile --enable-kernel=2.4.18 --enable-omitfp \
...
$(libc)-$(OPT): $(stamp_install_opt) debian/control $(mkdir)/sysdeps.mk \
  debian/libc/DEBIAN/shlibs
	...
	$(make_directory) $(tmpdir)/$@/lib/$(OPT_SUBDIR)
	$(INSTALL_DATA) $(install_root_opt)/lib/lib*-$(VERSION).so $(tmpdir)/$@/lib/$(OPT_SUBDIR)/.
	$(INSTALL_PROGRAM) $(install_root_opt)/lib/libc-$(VERSION).so $(tmpdir)/$@/lib/$(OPT_SUBDIR)/.
	$(INSTALL_DATA) $(install_root_opt)/lib/libSegFault.so $(tmpdir)/$@/lib/$(OPT_SUBDIR)/.
ifeq ($(threads),yes)
	$(INSTALL_DATA) $(install_root_opt)/lib/libpthread-0.10.so $(tmpdir)/$@/lib/$(OPT_SUBDIR)/.
	$(INSTALL_DATA) $(install_root_opt)/lib/libthread_db-1.0.so $(tmpdir)/$@/lib/$(OPT_SUBDIR)/.
endif
	@set -e; \
	cd $(install_root_opt)/lib; \
	for l in `find . -type l -name 'lib*.so.*'`; \
	do cp -vdf $$l $(tmpdir)/$@/lib/$(OPT_SUBDIR)/.; done
	cd $(tmpdir)/$@ && \
	$(STRIP) lib/$(OPT_SUBDIR)/lib*-$(VERSION).so
ifeq ($(threads),yes)
	$(STRIP) $(tmpdir)/$@/lib/$(OPT_SUBDIR)/libpthread-0.10.so
	$(STRIP) $(tmpdir)/$@/lib/$(OPT_SUBDIR)/libthread_db-1.0.so
endif
	# Get rid of the NSS libs, they don't get used anyway
	rm -f $(tmpdir)/$@/lib/$(OPT_SUBDIR)/libnss_*
